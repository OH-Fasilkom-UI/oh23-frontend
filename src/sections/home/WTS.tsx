                ...
                testimonials.map((testimonial, index) => (
                    <article key={index} onClick={() => setSelected(index)} className={`font-satoshi ${selected === index ? "w-max" : "w-1/4"} h-[350px] sm:h-[400px] flex border-2 border-white-2 p-5 rounded-md bg-shimmerGreen-2 bg-opacity-20 backdrop-blur-xl`}>
                      <aside className={`${selected !== index ? "flex-1" : "w-1/3"} min-w-[200px]`}>
                        ...
                        <div className="relative rounded-full bg-white-2 w-36 h-36 block mb-5">
                          <Image src={testimonial.imageUrl} alt="" fill className="rounded-full" />
                        </div>
                      ...
                      <aside className={`${selected === index ? "flex-1 px-5 sm:px-10 min-w-[500px]": "w-0"} transition-all duration-300 relative overflow-hidden text-left flex flex-col gap-3 justify-center`}>
                        ...
                        <p className={`${selected === index ? "opacity-100 delay-200" : "opacity-0 transition-none"} max-w-prose sm:max-w-prose text-white-1 transition-opacity duration-500`}>{testimonial.quote}</p>
