...
• Diundang oleh Kementerian Sekretariat Negara.
  ...
  return (
    <section className="flex flex-col items-center">
      {idx != -1 ? (
        <>
          <div className='relative mt-10 bg-[url(/wof/bg-profile-desktop.png)] h-[115vh] w-full sm:bg-[url(/wof/bg-profile-desktop.png)] bg-center bg-contain bg-no-repeat flex flex-col items-center justify-center'>
            <div className="mx-5 sm:max-h-[70vh] flex sm:flex-row flex-col backdrop-blur-lg sm:w-3/4 rounded-xl border-2 border-screaminGreen-3 p-5 sm:py-10 gap-4 sm:gap-8">
              <Image src={data[idx].img} alt="" width={250} height={250} className="mx-auto rounded-md sm:w-5/12 object-cover aspect-square"/>
              <div className="flex flex-col w-full gap-4 sm:gap-8">
                <h2 className="text-3xl text-center sm:text-left font-bold text-white-1">
                  {data[idx].name}
                </h2>
                <p className="text-white-1 overflow-auto max-h-[300px] whitespace-break-spaces">
                  {data[idx].desc}
                </p>
                <Button
                  onClick={() => setIdx(-1)}
            ...
            }} className="absolute top-1/2 -translate-y-1/2 left-0 sm:left-3 z-[1] text-white-1 aspect-square rounded-full bg-green-500 p-2 sm:text-3xl md:text-4xl text-2xl">
              <HiArrowLeft />
            </button>
            <button onClick={() => {
              setIdx(idx + 1 > data.length - 1 ? 0 : idx + 1)
            }} className="absolute top-1/2 -translate-y-1/2 right-0 sm:right-3 z-[1] text-white-1 aspect-square rounded-full bg-green-500 p-2 sm:text-3xl md:text-4xl text-2xl">
              <HiArrowRight />
            </button>
          </div>
        </>
        ...
        <>
          ...
          <div className='mt-10 bg-[url(/wof/bg-browse-mobile.jpg)] min-h-screen w-full sm:bg-[url(/wof/bg-browse-desktop.jpg)] bg-center sm:bg-contain bg-cover bg-no-repeat flex items-center justify-center'>
            <div className="overflow-auto sm:w-9/12 w-10/12 relative flex items-center gap-3 justify-between align-middle pl-5 sm:aspect-video aspect-[2/3] py-4">
              ...
              {data.map((item, index) => (
                <div
                  ...
                  key={index}
                  className="w-full h-full p-4 md:p-6 flex flex-col gap-y-5 items-center rounded-xl bg-cards backdrop-blur hover:cursor-pointer"
